...
namespace UnitTests.NodeTests
{
    [TestClass]
    public class NodeSettingsDataAccessServiceTests
    {
        ...
        public async Task TestGetNodesAsync()
        {
            ...
            Assert.AreEqual("https://LOUTFS01:5000", nodes[0].NodeApiUri);
        }

        [TestMethod]
        public async Task TestGetNodePackageAsync()
        {
            var nodePackages = (await _nodeSettingsDataAccess.GetPackagesAsync()).ToArray();
